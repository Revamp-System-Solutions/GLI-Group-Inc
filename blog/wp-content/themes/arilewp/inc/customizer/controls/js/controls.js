	...
	control: {
		...
		'arilewp-color': {
			...
			init: function ( control ) {
				...
				// Init the control.
				arilewp.input.color.init( control );
			},
			...
			template: function ( control ) {
				control.container.html(
					arilewp.input.color.getTemplate( {
		...
		color: {
			...
			getTemplate: function ( data ) {
				...
				html +=
					...
					'" value="' +
					data.value +
					'" class="arilewp-color-control" data-id="' +
					data['data-id'] +
					'"/>';

				return (
					'<div class="arilewp-input-container" data-id="' +
					data.id +
					'">' +
					html +
					'</div>'
			...
			init: function ( control ) {
				var picker = jQuery(
					'.arilewp-color-control[data-id="' + control.id + '"]'
				    ),
				    clear;
				...
				setTimeout( function () {
					clear = jQuery(
						'.arilewp-input-container[data-id="' +
						control.id +
						'"] .wp-picker-clear'
					);
					if ( clear.length ) {
...
(
	function () {
		 ...
		 */
		wp.customize.arilewpDynamicControl = wp.customize.Control.extend( {
			initialize: function ( id, options ) {
				...
				if ( !args.params.type ) {
					args.params.type = 'arilewp-generic';
				}
			...
			ready: function () {
				...
				control.deferred.embedded.done( function () {
					control.initArileWPControl( control );
				} );
			},
			...
			embed: function () {
				...
				wp.customize.section( sectionId, function ( section ) {
					if (
						'arilewp-expanded' === section.params.type ||
						section.expanded() ||
			 ...
			 * @returns {void}
			 */
			initArileWPControl: function ( control ) {
				if ( 'undefined' !== typeof arilewp.control[control.params.type] ) {
					arilewp.control[control.params.type].init( control );
					return;
				}
			...
			arilewpValidateCSSValue: function ( value ) {
				var validUnits = [
					    'rem',
					    'em',
					    'ex',
...
_.each( arilewp.control, function ( obj, type ) {
	wp.customize.controlConstructor[
		type
		] = wp.customize.arilewpDynamicControl.extend( {} );
} );
...
wp.customize.controlConstructor['arilewp-sortable'] = wp.customize.Control.extend( {
	...
	ready: function () {
		...
		// Init the control.
		if (
			!_.isUndefined( window.arilewpControlLoader ) &&
			_.isFunction( arilewpControlLoader )
		) {
			arilewpControlLoader( control );
		} else {
			control.initArileWPControl();
	...
	initArileWPControl: function () {
		...
		var control = this;
		control.container.find( '.arilewp-controls-loading-spinner' ).hide();
 ...
 * Control: Radio image.
 */
wp.customize.controlConstructor['arilewp-radio-image'] = wp.customize.arilewpDynamicControl.extend( {} );

/**
 * Control: Toggle.
 */
wp.customize.controlConstructor['arilewp-toggle'] = wp.customize.arilewpDynamicControl.extend( {
 ...
 */
wp.customize.controlConstructor['arilewp-radio-buttonset'] = wp.customize.Control.extend( {} );
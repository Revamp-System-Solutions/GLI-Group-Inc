                            ...
                            <tbody  v-if="posts.data.length > 0" class="bg-white divide-y divide-gray-200">
                                <tr v-for="post in posts.data" :key="post.id">
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <div class="text-sm font-bold text-gray-700">{{post.ratings}}</div>
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap w-2/5">
                                        <div class="text-sm text-gray-700">{{ post.client_name }}</div>
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <div class="text-sm font-bold text-gray-700">{{ post.client_org }}</div>
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap space-x-1 ">
                                        <inertia-link :href="$route('testimonial.edit', {id: post.id})" class="text-sm text-white bg-blue-400 px-3 py-1 rounded-sm" v-if="user" ><i class="fas fa-edit"></i> Edit</inertia-link>
                                        <a href="javascript:void(0);" class="text-sm text-white bg-red-400 px-3 py-1 rounded-sm" @click.prevent="openModal(post.id)" v-if="user"><i class="fas fa-trash-alt"></i> Delete</a>
                                    </td>
                                </tr>
                            </tbody>
                            <tbody v-else>
                                <tr class="text-center">
                                    <td colspan="4" class="text-center px-6 py-4 whitespace-nowrap">
                                        <div class="mb-2">No Testimonials found!</div>
        ...
        <div class="min-h-screen px-4 text-center">
          ...
          >
            ...
            >
              ...
              >
                <i class="fas fa-trash-alt text-red-700"></i>  Delete Testimonial #{{targetPort}}
              </DialogTitle>
              <div class="mt-2">
                <p class="text-sm text-gray-500">
                 Are you sure you want to this testimonial? This action will not affect Media used.
                </p>
              </div>
...
export default {
    ...
    setup() {
        ...
        const deletePost = (id) => {
            ...
            Inertia.delete(route('testimonial.destroy', {id}),{
               onSuccess: () =>{
                    isOpen.value = false
                }
            });
